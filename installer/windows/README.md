...
WiX 5 installer that installs **ncclient** (CLI), **ncclient-tray** (the older,
unelevated Python tray - menu + popup dialogs only), **NebulaCommanderApp**
(the windowed WinUI 3 app - side-tab Status/Enrollment/Settings UI, minimizes
to tray on close; this is the default GUI client now), and **ncclient-service**
...
- The four executables in `redist/`:
  - `redist/ncclient.exe`
...
- Installs all four exes to **Program Files\Nebula Commander** (per-machine).
...
- Creates `%ProgramData%\nebula-commander\` (shared settings, DPAPI-encrypted
  ...
  GUI clients can write there while the LocalSystem service reads/writes freely.
- Grants local `Authenticated Users` start/stop/query-status rights on the
  ...
  can control a service by default) - this is what lets the GUI clients'
...
- **Finish dialog**: "Launch Nebula Commander now" checkbox launches `NebulaCommanderApp.exe` (not the older tray).
- **Start Menu** shortcuts: "Nebula Commander (CLI)", "Nebula Commander" (the windowed app), and "Nebula Commander Tray (Classic)".
...
The GitHub Actions workflow builds the MSI after building the Windows ncclient, tray, service, and windowed-app exes, then uploads `NebulaCommander-windows-amd64.msi` to the release. See `.github/workflows/build-ncclient-binaries.yml`.
