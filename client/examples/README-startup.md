...
you (start type: Automatic) - pair it with `NebulaCommanderApp.exe` for a GUI
to enroll/configure it, and you don't need anything on this page. The two
options below are for a bare `pip install nebula-commander` CLI setup with no
GUI, using generic OS tools to keep `ncclient run` alive in the background
instead.
